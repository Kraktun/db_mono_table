...
/// Signature for [DataColumnR.onSort] callback.
typedef DataColumnRSortCallback = void Function(int columnIndex, bool ascending);

/// Column configuration for a [DataTable].
///
/// One column configuration must be provided for each column to
/// display in the table. The list of [DataColumnR] objects is passed
...
class DataColumnR {
  ...
  final DataColumnRSortCallback onSort;
...
class DataRowR {
  ...
  bool get _debugInteractive => onSelectChanged != null || cells.any((DataCellR cell) => cell._debugInteractive);
}

/// The data for a cell of a [DataTable].
///
/// One list of [DataCellR] objects must be provided for each [DataRowR]
/// in the [DataTable], in the new [DataRowR] constructor's `cells`
/// argument.
@immutable
class DataCellR {
  ...
  static const DataCellR empty = DataCellR(SizedBox(width: 0.0, height: 0.0));
  ...
  /// [DataRowR.onSelectChanged] is provided).
...
/// role. The columns are defined by three [DataColumnR] objects. The table
/// contains three rows of data for three example users, the data for which
/// is defined by three [DataRowR] objects.
///
...
///       DataRowR(
///         cells: <DataCellR>[
///           DataCellR(Text('Sarah')),
///           DataCellR(Text('19')),
///           DataCellR(Text('Student')),
///         ],
///       ),
///       DataRowR(
///         cells: <DataCellR>[
///           DataCellR(Text('Janine')),
///           DataCellR(Text('43')),
///           DataCellR(Text('Professor')),
///         ],
///       ),
///       DataRowR(
///         cells: <DataCellR>[
///           DataCellR(Text('William')),
///           DataCellR(Text('27')),
///           DataCellR(Text('Associate Professor')),
///         ],
///       ),
///     ],
///   );
...
///           }),
///           cells: [DataCellR(Text('Row $index'))],
...
/// See also:
///
///  * [DataColumnR], which describes a column in the data table.
///  * [DataRowR], which contains the data for a row in the data table.
///  * [DataCellR], which contains the data for a single cell in the data table.
///  * [PaginatedDataTable], which shows part of the data in a data table and
///    provides controls for paging through the remainder of the data.
///  * <https://material.io/design/components/data-tables.html>
class DataTableR extends StatelessWidget {
  /// Creates a widget describing a data table.
  ///
  /// The [columns] argument must be a list of as many [DataColumnR]
  ...
  ///
  /// The [rows] argument must be a list of as many [DataRowR] objects
  ...
  /// not be null.
  ///
  /// Each [DataRowR] object in [rows] must have as many [DataCellR]
  /// objects in the [DataRowR.cells] list as the table has columns.
  ///
  ...
  DataTableR({
    ...
    @required this.rows,
       ...
       assert(rows != null),
       assert(!rows.any((DataRowR row) => row.cells.length != columns.length)),
       assert(dividerThickness == null || dividerThickness >= 0),
       _onlyTextColumn = _initOnlyTextColumn(columns),
       super(key: key);

  /// The configuration and labels for the columns in the table.
  final List<DataColumnR> columns;
  ...
  ///
  /// If this is null, then the [DataRowR.onSelectChanged] callback of
  /// every row in the table is invoked appropriately instead.
  ///
  /// To control whether a particular row is selectable or not, see
  /// [DataRowR.onSelectChanged]. This callback is only relevant if any
  /// row is selectable.
  ...
  /// individual rows, see [DataRowR.color].
  ///
  ...
  /// selectable. However, if [DataRowR.onSelectChanged] is not set for any row,
  ...
  /// top and bottom padding of each row
  final List<double> verticalPadding; // REV

  // Set by the constructor to the index of the only Column that is
  // non-numeric, if there is exactly one, otherwise null.
  final int _onlyTextColumn;
  static int _initOnlyTextColumn(List<DataColumnR> columns) {
    int result;
    for (int index = 0; index < columns.length; index += 1) {
      final DataColumnR column = columns[index];
      if (!column.numeric) {
        if (result != null)
          return null;
        result = index;
  ...
  bool get _debugInteractive {
    return columns.any((DataColumnR column) => column._debugInteractive)
        || rows.any((DataRowR row) => row._debugInteractive);
  }
  ...
  void _handleSelectAll(bool checked) {
    if (onSelectAll != null) {
      ...
      for (final DataRowR row in rows) {
  ...
  Widget _buildDataCellR({
    ...
    label = Container(
      padding: padding,
      // REMOVED TO LET ROWS EXPAND
      //height: effectiveDataRowHeight, // REV
      alignment: numeric ? Alignment.centerRight : AlignmentDirectional.centerStart,
  ...
  Widget build(BuildContext context) {
    ...
    final bool anyRowSelectable = rows.any((DataRowR row) => row.onSelectChanged != null);
    final bool displayCheckboxColumn = showCheckboxColumn && anyRowSelectable;
    final bool allChecked = displayCheckboxColumn && !rows.any((DataRowR row) => row.onSelectChanged != null && !row.selected);
    final double effectiveHorizontalMargin = horizontalMargin
    ...
    for (int dataColumnIndex = 0; dataColumnIndex < columns.length; dataColumnIndex += 1) {
      final DataColumnR column = columns[dataColumnIndex];
      ...
      final EdgeInsetsDirectional padding = EdgeInsetsDirectional.only(
        ...
        top: verticalPadding[0] ?? 0, // REV
        bottom: verticalPadding[1] ?? 0, // REV
      );
      ...
      rowIndex = 1;
      for (final DataRowR row in rows) {
        final DataCellR cell = row.cells[dataColumnIndex];
        tableRows[rowIndex].children[displayColumnIndex] = _buildDataCellR(
          context: context,
          padding: padding,
          label: cell.child,
          numeric: column.numeric,
